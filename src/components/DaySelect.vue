<template>
    <div id="day-select">
        <ul class="days">
            <li v-bind:class="{day: true, active: isActive(day)}"  v-on:click="selectDay(day)" v-for="day in days">
                {{formatDay(day)}}
            </li>
            <li class="day-selector">
                <span v-on:click="changeDay(-1)" class="dec"></span>
                <span v-on:click="changeDay(+1)" class="inc"></span>
            </li>
        </ul>
    </div>
</template>
